...
export class LanguageClient {
	...
	private async analyzeFile(filePath: string): Promise<SymbolInfo[]> {
		...
		const symbolsPromise = this.connection.sendRequest(DocumentSymbolRequest.type, params) as Promise<DocumentSymbol[]>;
	...
	private extractDocumentation(lines: string[], symbolStartLine: number): string | undefined {
		...
		while (currentLine >= 0) {
			...
			if ((this.language === 'cpp' || this.language === 'c') && (line.startsWith('///') || line.startsWith('//!'))) {
	...
	private async getSupertypes(filePath: string, position: LSPPosition): Promise<string[] | undefined> {
		...
		try {
			...
			const supertypes = (await this.connection.sendRequest(TypeHierarchySupertypesRequest.type, supertypesParams)) as
				| TypeHierarchyItem[]
				| null;
